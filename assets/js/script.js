function saveAppointments(){
	...
	$("body").append('<div class="disScreen"><img src="/assets/images/ajax-loader.gif"></div>');
	$.ajax({method: "POST", dataType: "json",
		url: '/sendAppointments/',
		data: $("#frmAppointments").serialize(),
	})
	.done(function( data ) {
		if(document.querySelector('#msgAppointments').classList.contains('sub-title')){
			document.querySelector('#msgAppointments').classList.remove("sub-title");
		}
		if(document.querySelector('#msgAppointments').classList.contains('errormsg')){
			document.querySelector('#msgAppointments').classList.remove("errormsg");
		}
		
		if(data.savemsg=='Sent'){
			$("#msgAppointments").html('Your request sent successfully to Support. They will contact you soon.');			
			if(!document.querySelector('#msgAppointments').classList.contains('sub-title')){
				document.querySelector('#msgAppointments').classList.add("sub-title");
			}
		}
		else{
			$("#msgAppointments").html('Sorry! Your request could not sent to Support. Please try again.');
			if(!document.querySelector('#msgAppointments').classList.contains('errormsg')){
				document.querySelector('#msgAppointments').classList.add("errormsg");
			}
		}
		...
		if($(".disScreen").length){$(".disScreen").remove();}
	})
	.fail(function() {		
...
function sendContactUs(event){
	...
	.done(function( data ) {
		...
		if(data.returnStr=='sent'){			
			HaveAnyQuestionSpan.innerHTML = 'Your request sent successfully to Support. They will contact you soon.';
			$("#contactUsForm").find("select, input, textarea").val("");		
		}
		else{
			HaveAnyQuestionSpan.setAttribute('class', 'errorMsg');
			HaveAnyQuestionSpan.innerHTML = 'Sorry! Your request could not sent to Support. Please try again.';
		}
...
function loadMore(start){
	...
	$.ajax({
		...
		success:function(result){
			// console.log(result);
			var loadCalling = "<a style='border:0px solid red; margin:0 auto;' class='btn btn-info' id='loadMoreBtn' href='#' onclick='call_lazy_load(event)'>Load More<a>";
			jQuery('#block').append(result);
			// jQuery('#loadbtn').html(loadCalling);
			load_flag+=1;
		}
	});
}

function call_lazy_load(e){
	e.preventDefault();
	loadMore(load_flag);
...
$(document).ready(function(){
	...
	$(window).scroll(function(){
		...
		timeout = setTimeout(function() {
			if($(window).scrollTop() >= $(document).height() - ($(window).height()+690))
		{
			loadMore(load_flag);
		}
		}, 50);  
