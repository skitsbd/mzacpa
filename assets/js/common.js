    ...
    .done(function (data) {
      if (data.login != "") {
        ...
        var formhtml =
          ...
          frompage +
          '">' +
          '<input type="hidden" name="teams_id" value="' +
          customers_id +
          '">' +
          "</form>" +
          "</div>";
...
function AJsave_Customers() {
  ...
  var customers_id = document.frmcustomer.teams_id.value;
  ...
  j.ajax({
    ...
    .done(function (data) {
      ...
      if (data.savemsg != "error") {
        if (data.teams_id > 0) {
          window.location = "/Customers/view/" + data.teams_id;
        } else {
...
function filter_Teams_lists() {
  ...
  j.ajax({
    ...
    .fail(function () {
      connection_dialog(filter_Teams_lists);
    });
}

function loadTableRows_Teams_lists() {
  var jsonData = {};
  jsonData["sorting_type"] = j("#sorting_type").val();
  jsonData["keyword_search"] = j("#keyword_search").val();
  jsonData["totalRows"] = j("#totalTableRows").val();
  ...
  j.ajax({
    ...
    .fail(function () {
      connection_dialog(loadTableRows_Teams_lists);
    });
}

function AJget_TeamsPopup(teams_id) {
  var frompage = j("#segment1").val();
  j("body").append(
    '<div class="disScreen"><img src="/assets/images/ajax-loader.gif"></div>'
  );
  ...
  j.ajax({
    ...
    .done(function (data) {
      if (data.login != "") {
        ...
        var formhtml =
          '<div class="floatleft padding0 maxheight600">' +
          '<div id="error_team" class="errormsg"></div>' +
          '<form action="#" name="frmteam" id="frmteam" onsubmit="return AJsave_Teams();" enctype="multipart/form-data" method="post" accept-charset="utf-8">' +
          '<div class="col-sm-12">' +
          '<div class="form-group row">' +
          '<div class="col-sm-4" align="left">' +
          '<label for="name">Name<span class="required">*</span></label>' +
    ...
    .fail(function () {
      connection_dialog(AJget_TeamsPopup, teams_id);
    });

  return true;
}

function AJsave_Teams() {
  var error_team = document.getElementById("error_team");
  error_team.innerHTML = "";
  if (j("#name").val() == "") {
    error_team.innerHTML = "Missing first name";
    j("#name").focus();
    return false;
  }
  var requiredFields = document.getElementsByClassName("required");
  if (requiredFields.length > 0) {
    for (var l = 0; l < requiredFields.length; l++) {
      var oneFieldVal = requiredFields[l].value;
      if (oneFieldVal == "") {
        error_team.innerHTML = requiredFields[l].title + " is missing.";
        requiredFields[l].focus();
        return false;
      }
    }
  ...
  var teams_id = document.frmteam.teams_id.value;
  ...
  j.ajax({
    method: "POST",
    dataType: "json",
    url: "/Teams/aJsave_Teams/",
    data: j("#frmteam").serialize(),
  })
    .done(function (data) {
      if (data.login != "") {
        window.location = "/" + data.login;
      ...
      if (data.savemsg != "error") {
        if (data.teams_id > 0) {
          window.location = "/Teams/view/" + data.teams_id;
        } else {
          document.getElementById("error_team").innerHTML = data.message;
        }
      } else {
        document.getElementById("error_team").innerHTML = data.message;
      }
    ...
    .fail(function () {
      connection_dialog(AJsave_Teams);
    });
  return false;
}

function AJmergeTeamsPopup(teams_id) {
  if (teams_id > 0) {
    j("body").append(
      '<div class="disScreen"><img src="/assets/images/ajax-loader.gif"></div>'
    );
    j.ajax({
      ...
      .done(function (data) {
        if (data.login != "") {
          ...
          var formhtml =
            '<div id="error_team" class="errormsg"></div>' +
            '<form action="#" name="frmMergeTeam" id="frmMergeTeam" onsubmit="return AJmergeTeams();" enctype="multipart/form-data" method="post" accept-charset="utf-8">' +
            '<div class="form-group row">' +
            '<div class="col-sm-12" align="left">' +
            "<h4>Merge this team information</h4>" +
            "</div>" +
            "</div>" +
            '<div class="form-group row">' +
            '<div class="col-sm-12 image_content txtleft">' +
            ...
            '<div class="form-group row">' +
            '<div class="col-sm-2" align="left">' +
            '<label for="team_name">Name<span class="required">*</span></label>' +
            "</div>" +
            '<div class="col-sm-10">' +
            '<input maxlength="50" type="text" value="" required name="team_name" id="team_name" class="form-control" placeholder="Search Teams">' +
            "</div>" +
            "</div>" +
            '<div class="form-group row">' +
            '<div class="col-sm-12 image_content txtleft" id="toTeamInfo"></div>' +
            "</div>" +
            '<input type="hidden" name="fromteams_id" id="fromteams_id" value="' +
            teams_id +
            '">' +
            '<input type="hidden" name="toteams_id" id="toteams_id" value="0">' +
            "</form>";

          form600dialog(
            "Merge the following two teams",
            formhtml,
            "Merge Teams",
            AJmergeTeams
          );
      ...
      .fail(function () {
        connection_dialog(AJmergeTeamsPopup, teams_id);
      });
  }
  return true;
}

function AJmergeTeams() {
  j("#error_team").html("");
  if (j("#toteams_id").val() == 0) {
    j("#error_team")
      .html(
        '<div class="col-sm-12 col-sm-12 col-md-12"><div class="bs-callout bs-callout-info well alert_msg">Merging teams is empty. Please search and choose different team.</div></div>'
      )
      .fadeIn(500);
    return false;
  }
  if (j("#fromteams_id").val() == j("#toteams_id").val()) {
    j("#error_team")
      .html(
        '<div class="col-sm-12 col-sm-12 col-md-12"><div class="bs-callout bs-callout-info well alert_msg">Merging teams is same. Please choose different team.</div></div>'
      )
      .fadeIn(500);
    return false;
  }

  j(".btnmodel").html("Merging Teams...").prop("disabled", true);
  ...
  j.ajax({
    method: "POST",
    dataType: "json",
    url: "/Teams/aJmergeTeams/",
    data: j("#frmMergeTeam").serialize(),
  })
    .done(function (data) {
      if (data.login != "") {
        window.location = "/" + data.login;
      } else if (data.savemsg == "Success" && data.id > 0) {
        window.location = "/Teams/view/" + data.id;
      } else {
        j(".btnmodel").html("Merge Teams").prop("disabled", false);
        j("#showmessagehere")
          .html(
            '<div class="col-sm-12 col-sm-12 col-md-12"><div class="bs-callout bs-callout-info well alert_msg">There is an error while merging teams information.</div></div>'
          )
          .fadeIn(500);
        setTimeout(function () {
          j("#showmessagehere").slideUp(500);
    ...
    .fail(function () {
      connection_dialog(AJmergeTeams);
    });
  return false;
}
...
function filter_Teams_view() {
  var limit = j("#limit").val();
  var page = 1;
  j("#page").val(page);
  var jsonData = {};
  jsonData["teams_id"] = j("#table_idValue").val();
  jsonData["shistory_type"] = j("#shistory_type").val();
  jsonData["totalRows"] = j("#totalTableRows").val();
  jsonData["rowHeight"] = j("#rowHeight").val();
  jsonData["limit"] = limit;
  ...
  j.ajax({
    ...
    .fail(function () {
      connection_dialog(filter_Teams_view);
    });
}

function loadTableRows_Teams_view() {
  var jsonData = {};
  jsonData["teams_id"] = j("#table_idValue").val();
  jsonData["shistory_type"] = j("#shistory_type").val();
  jsonData["totalRows"] = j("#totalTableRows").val();
  ...
  j.ajax({
    ...
    .fail(function () {
      connection_dialog(loadTableRows_Teams_view);
    });
}
...
function archiveTeamconfirm() {
  var teams_id = document.getElementById("teams_id");
  var team_name = document.getElementById("team_name");
  j(".archive").slideUp(500);

  archiveData(
    "/Manage_Data/jqueryteams_archive/",
    { team_name: team_name.value, teams_id: teams_id.value },
    "Team"
  );
  j("#dialog-confirm").dialog("close");
  team_name.value = "";
...
function display_image(e) {
  document
    .getElementById("servicesImage")
    .setAttribute("src", e.getAttribute("src"));
